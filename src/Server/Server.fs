...
let awsEnvCredentials () =
    ...
    if credentials.ExitCode <> 0 then
        failwith
            $"Error while getting AWS credentials using 'aws configure export-credentials`: {credentials.StandardError}"
    else
...
let rec searchAws' (request: SearchRequest) (region: string) (nextToken: string option) =
    task {
        let resources = ResizeArray()
        let explorer = resourceExplorerClient region

        nextToken
        |> Option.iter (fun token -> request.NextToken <- token)
...
let normalizeModuleName (input: string) =
    ...
    if parts.Length = 1 then
        ...
        [ for (i, part) in Array.indexed parts -> if i = 0 then part else capitalize part ]
        |> String.concat ""
...
let awsImportDocs (pulumiType: string) =
    ...
    | [| _; moduleName; _ |] ->
        ...
        | [| moduleName; resourceName |] ->
            let url =
                $"https://www.pulumi.com/registry/packages/aws/api-docs/{moduleName.ToLower()}/{resourceName.ToLower()}"
...
let importWarningDocsMarkdown (pulumiType: string, resourceId: string) =
    let docsInfo =
        ...
        |> Option.map (fun docs ->
            $"See [docs]({docs.url}) for more information about **aws.{docs.moduleName}.{docs.resourceName}** resource.")
        |> Option.defaultValue ""

    let warning =
        $"""Resource '**{pulumiType}**' with used its ID '**{resourceId}**' as the import ID.
...
let searchAws (request: AwsSearchRequest) =
    task {
        try
            let queryString =
...
let rec awsCloudFormationStacks (nextToken: string option) (region: string) =
    task {
...
let rec stackExports (nextToken: string option) (region: string) : Task<ResizeArray<Export>> =
    task {
...
let getStackExports (region: string) =
    task {
...
let getAwsCloudFormationStacks (region: string) =
    task {
        try
...
let getAwsCloudFormationGeneratedTemplates (region: string) =
    task {
        try
            let client = cloudFormationClient region
...
let cloudFormationResourceToSkip =
    set [ "AWS::CloudFormation::CustomResource"
          "AWS::CDK::Metadata"
...
let getJObject (key: string) (json: JObject) =
    if json.ContainsKey key
       && json[key].Type = JTokenType.Object then
        json[key] :?> JObject
    else
        JObject()

let getAwsCloudFormationGeneratedTemplate (request: AwsGeneratedTemplateRequest) =
    task {
        try
            let client = cloudFormationClient request.region

            let! templateResponse =
                client.GetGeneratedTemplateAsync(
                    GetGeneratedTemplateRequest(GeneratedTemplateName = request.templateName)
                )

            let! templateDetails =
                client.DescribeGeneratedTemplateAsync(
                    DescribeGeneratedTemplateRequest(GeneratedTemplateName = request.templateName)
                )
            ...
            for resource in templateDetails.Resources do
                ...
                for property in resourceProperties.Properties() do
                    if
                        ...
                        && not (resourceData.ContainsKey property.Name)
                    then
                        resourceData.Add(property.Name, property.Value.ToObject<string>())
                ...
                if includeResource then
                    ...
                    | None ->
                        ...
                        | Some pulumiType ->
                            ...
                            if resource.ResourceIdentifier.Count > 0 then
                                ...
                                errors.Add
                                    $"CloudFormation resource '{resource.ResourceType}' does not have an identifier"
                        | None ->
                            resourceJson.Add("type", resource.ResourceType)

                            errors.Add
                                $"CloudFormation resource '{resource.ResourceType}' did not have a corresponding Pulumi type"
                    ...
                    | Some pulumiMapping ->
                        ...
                        let importPartValues =
                            ...
                            |> Seq.choose (fun part ->
                                match resourceDataByLogicalID[resource.LogicalResourceId]
                                          .TryGetValue(part)
                                    with
            ...
            return
                Ok
                    { templateBody = templateBody.ToString(Formatting.Indented)
                      resourceDataJson =
                        (JObject.FromObject resourceDataByLogicalID)
...
let getImportIdentityParts (resourceType: string) : Option<seq<string>> =
    match AwsCloudFormationTemplates.remapSpecifications.TryFind resourceType with
    | Some (spec) -> Some spec.importIdentityParts
    | _ -> None

let shouldCheckPropNameForReferenceProperty (propName: string) (resourceType: string) : bool =
    ...
    | _ -> hasIdentifierSuffix

let getResource (identifier: string) (typeName: string) (region: string) =
    task {
...
// ]
let rec resolveAttPath (properties: JObject) (attPath: seq<string>) : string =
    if (Seq.length attPath) = 1 then
        ...
        if properties.ContainsKey key then
            ...
            if value.Type = JTokenType.String
               || value.Type = JTokenType.Integer then
                value.ToString()
            else
                ""
        else
    ...
    if token.Type = JTokenType.Object then
        ...
        if obj.ContainsKey "Ref" then
            ...
            if data.ContainsKey referencedResourceLogicalId then
                if data[referencedResourceLogicalId]
                    .ContainsKey IdProperty then
                    data [ referencedResourceLogicalId ] [
                        IdProperty
                    ]
                else
                    ""
        ...
        elif obj.ContainsKey "Fn::GetAtt"
             && obj["Fn::GetAtt"].Type = JTokenType.Array then
            let getAtt = obj[ "Fn::GetAtt" ].ToObject<JArray>()

            if getAtt.Count = 2
               && getAtt[0].Type = JTokenType.String
               && getAtt[1].Type = JTokenType.String then
                ...
                if data.ContainsKey referencedResourceLogicalId then
                    ...
                    if (Seq.length targetAttPath) = 1
                       && targetResourceData.ContainsKey key then
        ...
        // evaluating a Fn::Join requires recursive evaluation of the join parts
            ...
            if join.Count = 2
               && join[0].Type = JTokenType.String
               && join[1].Type = JTokenType.Array then
                let delimiter = join[ 0 ].ToObject<string>()
                let joinParts = join[ 1 ].ToObject<JArray>()

                let resolvedParts =
                    joinParts
                    |> Seq.map (fun part -> resolveTokenValue data stackExports part)
...
let templateBodyData
    ...
    let bodyJson =
        if isValidJson cloudformationTemplate.TemplateBody then
            JObject.Parse cloudformationTemplate.TemplateBody
        else
            Yaml.convertToJson cloudformationTemplate.TemplateBody
    ...
    for property in resourcesFromTemplate.Properties() do
        ...
        if not (data.ContainsKey logicalId) then
            data.Add(logicalId, Dictionary<string, string>())
        ...
        for property in properties.Properties() do
            if shouldCheckPropNameForReferenceProperty property.Name resourceType then
                ...
                data[logicalId]
                    .Add(property.Name, referenceValue)

    data, bodyJson

let resolveFnGetAtt (data: Dictionary<string, Dictionary<string, string>>) =
    task {
...
let getRouteTables (resourceTypes: Set<string>) (region: string) =
    task {
...
let getElasticIps (resourceTypes: Set<string>) (region: string) =
    task {
...
let getIamPolicies (resourceTypes: Set<string>) (region: string) =
    task {
...
let getLoadBalancers (resourceTypes: Set<string>) (region: string) =
    task {
...
let getInternetGateways (vpcId: string) (region: string) =
    task {
        let client = ec2Client region
        let request = DescribeInternetGatewaysRequest()

        request.Filters <-
            ResizeArray [ Amazon.EC2.Model.Filter(Name = "attachment.vpc-id", Values = ResizeArray[vpcId]) ]

        let! response = client.DescribeInternetGatewaysAsync(request)
        return response.InternetGateways
    }

let getImportIdsForVPCGatewayAttachments (cloudformationResources: seq<AwsCloudFormationResource>) (region: string) =
    task {
        ...
        for resource in vpcGatewayAttachments do
            ...
            | [| _; vpcId |] ->
                ...
                | Some gateway -> data.Add(resource.logicalId, $"{gateway.InternetGatewayId}:{vpcId}")
                | None -> data.Add(resource.logicalId, resource.resourceId)
            | _ -> data.Add(resource.logicalId, resource.resourceId)
...
let resourceTypeInTemplate (resourceType: string) (cloudformationResources: seq<AwsCloudFormationResource>) : bool =
    let count =
        cloudformationResources
    ...
    for resource in cloudformationResources do
        ...
        | _ ->
            ...
            if resource.resourceType = cloudFormationLoadBalancerType then
                ...
                | _ -> resourceJson.Add("type", "aws:elb/loadBalancer:LoadBalancer")
            else
                // set type as pulumi type from AwsCloudFormation.mapToPulumi (autogenerated from ?)
                match AwsCloudFormation.mapToPulumi resource.resourceType with
                | Some pulumiType -> resourceJson.Add("type", pulumiType)
                | None ->
                    resourceJson.Add("type", resource.resourceType)
            ...
            if resource.resourceType = "AWS::ApiGateway::Method" then
                ...
                | true, eip -> resourceJson.Add("id", eip.AllocationId)
                | _ -> resourceJson.Add("id", resource.resourceId)
                ...
                | Some policy -> resourceJson.Add("id", policy.Arn)
                | _ -> resourceJson.Add("id", resource.resourceId)
                ...
                | true, importId -> resourceJson.Add("id", importId)
                | _ -> resourceJson.Add("id", resource.resourceId)
...
let getAwsCloudFormationResources (stack: AwsCloudFormationStack) =
    task {
        try
            ...
            let resourceData, bodyJson =
                templateBodyData stackTemplate cloudformationResources stackExports stack.region
            ...
            let resourceTypes =
                Set [ for resource in cloudformationResources -> resource.resourceType ]
            ...
            let! securityGroupIngressRules =
                listResourcesIfInTemplate cloudformationResources "AWS::EC2::SecurityGroupIngress" stack.region

            let! securityGroupEgressRules =
                listResourcesIfInTemplate cloudformationResources "AWS::EC2::SecurityGroupEgress" stack.region
            ...
            let (pulumiImportJson, errors) =
                getPulumiImportJson cloudformationResources resourceContext resourceData
            ...
            return
                Ok
                    { resources = List.ofSeq cloudformationResources
                      ...
                      resourceDataJson =
                        (JObject.FromObject resourceData)
                            .ToString(Formatting.Indented) }
        with
        | error ->
            let errorType = error.GetType().Name
...
let pulumiCliBinary () : Task<string> =
    task {
        try
...
let invalidTypesInImportJson (json: string) =
    ...
    if parsedJson.ContainsKey "resources"
       && parsedJson.["resources"].Type = JTokenType.Array then
        ...
        |> Seq.choose (fun resource ->
            ...
            if resourceType.StartsWith "AWS::"
               || resourceType.Split(":").Length <> 3 then
                Some resourceType
            else
                None)
        |> Seq.distinct
...
let importPreview (request: ImportPreviewRequest) =
    task {
        try
                    ...
                    let response =
                        ...
                        <| fun tempDir ->
                            let exec (args: string) =
                                Cli
                                    ...
                                    .WithEnvironmentVariables(fun config ->
                                        if request.region <> ""
                                           && request.region <> unsetDefaultRegion then
                                            config
                            ...
                            if pulumiNewOutput.ExitCode <> 0 then
                                Error
                                    $"Error occurred while running 'pulumi {newCommandArgs}' command: {pulumiNewOutput.StandardError}"
                            else
                                ...
                                if pulumiLoginOutput.ExitCode <> 0 then
                                    Error
                                        $"Error occurred while running 'pulumi login file://./state' command: {pulumiLoginOutput.StandardError}"
                                else
                                    ...
                                    if initStack.ExitCode <> 0 then
                                        Error
                                            $"Error occurred while running 'pulumi stack init dev' command: {initStack.StandardError}"
                                    else
                                        ...
                                        let pulumiImportOutput =
                                            exec $"import --file {importFilePath} --yes --out {generatedCodePath}"

                                        let hasErrors = pulumiImportOutput.StandardOutput.Contains "error:"

                                        if pulumiImportOutput.ExitCode <> 0 && hasErrors then
                                            Error
                                                $"Error occurred while running 'pulumi import --file <tempDir>/import.json --yes --out <tempDir>/generated.txt' command: {pulumiImportOutput.StandardOutput}"
                                        else
                                            ...
                                            if exportStackOutput.ExitCode <> 0 then
                                                Error
                                                    $"Error occurred while running 'pulumi stack export --file {stackOutputPath}' command: {exportStackOutput.StandardError}"
                                            else
                                                            ...
                                                            && not
                                                                (
                                                                    String.IsNullOrWhiteSpace
                                                                        pulumiImportOutput.StandardError
                                                                )
                                                        then
                                                            Some pulumiImportOutput.StandardError
                                                        else
                                                            None }
...
let importerApi =
    { getPulumiVersion = getPulumiVersion >> Async.AwaitTask
      ...
      getResourcesUnderResourceGroup =
        Azure.getResourcesUnderResourceGroup
        >> Async.AwaitTask
      ...
      getAwsCloudFormationGeneratedTemplates =
        getAwsCloudFormationGeneratedTemplates
        >> Async.AwaitTask
      getAwsCloudFormationGeneratedTemplate =
        getAwsCloudFormationGeneratedTemplate
        >> Async.AwaitTask
